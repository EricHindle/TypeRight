Public Class NCrypt
    ...
    Private Declare Function CryptAcquireContext Lib "advapi32.dll" Alias "CryptAcquireContextA" _
                                             (ByRef phProv As Long,
                                              ByVal pszContainer As String,
                                              ByVal pszProvider As String,
                                              ByVal dwProvType As Long,
    ...
    Private Declare Function CryptEncrypt Lib "advapi32.dll" _
                                      (ByVal hKey As Long,
                                       ...
                                       ByVal Final As Long,
                                       ByVal dwFlags As Long,
                                       ByVal pbData As String,
                                       ByRef pdwDataLen As Long,
    ...
    '*******************************************************************************
    Private Function EncryptDecrypt(ByVal sText As String,
                                ByVal sKeyRoot As String,
        ...
        If lResult = 0 Then
            Err.Raise(frezCryptoErrors.frezErrorCreatingHash, , ERROR_CREATING_HASH)
        End If
        ...
        If lResult = 0 Then
            Err.Raise(frezCryptoErrors.frezErrorCreatingHashData, , ERROR_CREATING_HASH_DATA)
        End If

        ' Select appropriate encryption method
        If enEncryptionType = frezCryptoEncryptionType.frezBlockEncryption Then
        ...
        If lResult = 0 Then
            Err.Raise(frezCryptoErrors.frezErrorDerivingKey, , ERROR_DERIVING_KEY)
        End If
        ...
        If bEncrypt Then
            ...
            sText = sText & CStr(lData - lClear)
            ' Encrypt some text
            lResult = CryptEncrypt(lKey, 0, 1, 0, sText, lClear, lData)
            If lResult = 0 Then
                Err.Raise(frezCryptoErrors.frezErrorEncryptingData, , ERROR_ENCRYPTING_DATA)
            End If
        Else
            ...
            If lResult = 0 Then
                Err.Raise(frezCryptoErrors.frezErrorDecryptingData, , ERROR_DECRYPTING_DATA)
            End If
        End If
...
ERROR_HANDLER:
    ...
    ' stream.
    '*******************************************************************************
    Public Function Encrypt(ByVal sTextToEncrypt As String,
                        ByVal sPrivateKey As String,
                        Optional ByVal bReturnTextInHex As Boolean _
                        = False,
                        Optional ByVal enEncryptionType As frezCryptoEncryptionType _
                        = frezCryptoEncryptionType.frezBlockEncryption) As String
        Dim sEncryptedText As String

        If enEncryptionType <> frezCryptoEncryptionType.frezBlockEncryption And enEncryptionType <> frezCryptoEncryptionType.frezStreamEncryption Then
            Err.Raise(frezCryptoErrors.frezErrorBadEncryptionType, "FrezCrypo.CCrypto.Encrypt", ERROR_BAD_ENCRYPTION_TYPE)
        End If

        If sTextToEncrypt = "" Or sPrivateKey = "" Then
            Err.Raise(frezCryptoErrors.frezErrorMissingParameter, "FrezCrypo.CCrypto.Encrypt", ERROR_MISSING_PARAMETER)
        End If
    ...
    '*******************************************************************************
    Public Function Decrypt(ByVal sTextToDecrypt As String,
                        ByVal sPrivateKey As String,
        ...
        If enEncryptionType <> frezCryptoEncryptionType.frezBlockEncryption And enEncryptionType <> frezCryptoEncryptionType.frezStreamEncryption Then
            ...
            Err.Raise(frezCryptoErrors.frezErrorBadEncryptionType, "FrezCrypo.CCrypto.Decrypt", ERROR_BAD_ENCRYPTION_TYPE)
        End If

        If sTextToDecrypt = "" Or sPrivateKey = "" Then
            Err.Raise(frezCryptoErrors.frezErrorMissingParameter, "FrezCrypo.CCrypto.Decrypt", ERROR_MISSING_PARAMETER)
        End If
    ...
    Private Function ConvertStringFromHex(ByVal sText As String) As String
        ...
        If lLength Mod 2 <> 0 Then
            Err.Raise(frezCryptoErrors.frezErrorInvalidHexString,
                  "FrezCrypo.CCrypto.ConvertStringFromHex",
    ...
    Private Sub Class_Initialize()
        ...
        lResult = CryptAcquireContext(m_lProvider, vbNullString,
                                  CRYPTO_PROVIDER, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)

        If lResult = 0 Then
            Err.Raise(frezCryptoErrors.frezErrorAquiringContext,
                  "FrezCrypo.CCrypto.Class_Initialize",
